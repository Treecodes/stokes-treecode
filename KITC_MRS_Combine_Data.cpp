...
#pragma omp parallel
{
	  ...
	  if (threadID==0) printf("Number of threads in direct sum: %i\n", omp_get_num_threads());
...
vec_3d Compute_RBF(size_t particle_index, size_t panel_index)
{
  // Input :
  //         particle_index
  //         panel_index
  ...
  if (tree[panel_index].MAC < R_sq)
    ...
    if (limit_2 - limit_1 < N0) // Otherwise, if cluster is a leaf, use direct sum
      ...
      for (size_t i = 0; i < length; i++) {
        ...
        vec_3d temp_result = Compute_RBF(particle_index, index);
...
int main()
{
  ...
  sprintf(point_data_file, "./combine_rand_lambda_%d.txt", numpars);
...
for (int i = 0; i < 3; i++)
  ...
  while (fscanf(fp, "%lf %lf %lf %lf %lf %lf", &x1, &x2, &x3, &c1, &c2, &c3) == 6) {
  ...
  */
  ...
  build_tree_time = getTickCount() - total_time;
  ...
  // Skip root
#pragma omp parallel // begin the paralell region.  Spawns OMP_NUM_THREADS threads
  {
  #pragma omp for schedule(guided)
  for (size_t i = 1; i < tree_size; i++)
    Panel_Moment_B(i, tree[i].moments);
  }
  moment_cpu_time = getTickCount() - moment_cpu_time;

  //***************** Compute Velocity ***************************

#pragma omp parallel // begin the parall region.  Spawns OMP_NUM_THREADS threads
  {
  int threadID = omp_get_thread_num();  // each thread gets its own ID.
  if (threadID==0){
	  cout << "Number of threads in Treecode: " << omp_get_num_threads() << endl;
  }
  ...
  cout << "moment_cpu_time " << moment_cpu_time << endl;
  ...
  cout << endl;
  cout << setprecision(5) << "E is " << E << endl;
  cout << setprecision(5) << "max_n is " << max_n << endl;
  cout << setprecision(5) << "max_d is " << max_d << endl;
  ...
  cout << endl;
  cout << setprecision(5) << "E_2_ex is " << err2_ex << endl;
