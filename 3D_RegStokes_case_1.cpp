...
static const int P = 10; // order of Taylor approximation
static const int Pflat = (P + 1)*(P + 1)*(P + 1);
static const int N_cube = 80000; // N_cube points total
static const int N = 80;
static const int N0 = 2000;
static const double sq_theta = 0.25; // theta = 0.5
static const double DEL = 0.02;
...
void build_tree_init()
{
	...
	temp_panel.xinterval[0] = -5.0; // interval defining the panel
	temp_panel.xinterval[1] = 5.0;
	temp_panel.yinterval[0] = -5.0;
	temp_panel.yinterval[1] = 5.0;
	temp_panel.zinterval[0] = 0.0;
	temp_panel.zinterval[1] = 10.0;
...
//*****************************************************************************//
void Panel_Moment_B(size_t panel_index, double *lambda[3], struct xyz &particles,
                    double m[][Pflat])
{
...
{
   /* double G[P + 1][P + 1][P + 1] = {{{0.0}}};
    double Bxx[P + 1][P + 1][P + 1] = {{{0.0}}};
    double Bxy[P + 1][P + 1][P + 1] = {{{0.0}}};
    double Bxz[P + 1][P + 1][P + 1] = {{{0.0}}};
    double Byy[P + 1][P + 1][P + 1] = {{{0.0}}};
    double Bzz[P + 1][P + 1][P + 1] = {{{0.0}}};
    double Byz[P + 1][P + 1][P + 1] = {{{0.0}}};
    */
    ...
    for (int i = 0; i < P + 1; i++)
    {
        ...
        for (int j = 0; j < P + 1; j++)
        {
            ...
            for (int k = 0; k < P + 1; k++)
            {
                ...
                invR = 1/ sqrt(R2 + DEL * DEL);
                invR3 = invR * invR * invR;
                
                G = -(2 * R2 + 3 * DEL * DEL) * invR3;
                velocity.val[0] += temp_moments[0] * (-G - invR);
                velocity.val[1] += temp_moments[1] * (-G - invR);
                velocity.val[2] += temp_moments[2] * (-G - invR);
                
                DD = temp_moments[0] * dx[i] + temp_moments[1] * dy[j] + temp_moments[2] * dz[k];
                DD = DD * invR3;
                 velocity.val[0] +=  DD * dx[i]; // part of Bxx + Bxy + Bxz
                 velocity.val[1] +=  DD * dy[j];
                 velocity.val[2] +=  DD * dz[k];
                
              /*
                G[i][j][k] = - (2 * R2 + 3 * DEL * DEL) * invR3;
                Bxx[i][j][k] = - (R2 + DEL * DEL - temp_i) * invR3;
                Byy[i][j][k] = - (R2 + DEL * DEL - temp_j) * invR3;
                Bzz[i][j][k] = - (R2 + DEL * DEL - temp_k) * invR3;
                Bxy[i][j][k] = ((x - t1[i]) * (y - t2[j])) * invR3;
                Bxz[i][j][k] = ( z - t3[k]) * (x - t1[i]) * invR3;
                Byz[i][j][k] = ( z - t3[k]) * (y - t2[j]) * invR3;
               */
    ...
    for (int i = 0; i < P + 1; i++)
    {
        for (int j = 0; j < P + 1; j++)
        {
            for (int k = 0; k < P + 1; k++)
            {
                ...
                velocity.val[0] += (-G[i][j][k] + Bxx[i][j][k]) * temp_moments[0] + Bxy[i][j][k] *  temp_moments[1] + Bxz[i][j][k] * temp_moments[2];
                
                velocity.val[1] += (-G[i][j][k] + Byy[i][j][k]) * temp_moments[1] + Bxy[i][j][k] *  temp_moments[0] + Byz[i][j][k] *  temp_moments[2];
                
                velocity.val[2] += (-G[i][j][k] + Bzz[i][j][k]) * temp_moments[2] + Bxz[i][j][k] *  temp_moments[0] + Byz[i][j][k] *  temp_moments[1];
            }
        }
    }
    */
...
vec_3d Call_Ds(int limit_1, int limit_2, int particle_index, double p_x, double p_y, double p_z, struct xyz &particles, double *lambda[3])
{
    ...
    for (size_t jj = limit_1; jj <= limit_2; jj++)
    {
        ...
        G = -(2 * R2 + 3 * DEL * DEL) * Rinv3;
        
        velocity.val[0] += ff[0] * (-G - Rinv);
        velocity.val[1] += ff[1] * (-G - Rinv);
        velocity.val[2] += ff[2] * (-G - Rinv);
        
        DD = ff[0] * x + ff[1] * y + ff[2] * z;
        DD = DD * Rinv3;
        
        velocity.val[0] +=  DD * x; // part of Bxx + Bxy + Bxz
        velocity.val[1] +=  DD * y;
        velocity.val[2] +=  DD * z;
        ...
        velocity.val[0] += (a[0][0] * ff[0] + a[0][1] * ff[1] + a[0][2]* ff[2]) * R3inv;
        velocity.val[1] += (a[1][0] * ff[0] + a[1][1] * ff[1] + a[1][2]* ff[2]) * R3inv;
        velocity.val[2] += (a[2][0] * ff[0] + a[2][1] * ff[1] + a[2][2]* ff[2]) * R3inv;
         */
    ...
    for (size_t i = 0; i < N_cube; i++)
    {
        ...
        for (size_t j = 0; j < N_cube; j++)
        {
            ...
            if (i != j)
            {
                ...
                DD = DD * Rinv3;
                sum[0] +=  DD * xx; // part of Bxx + Bxy + Bxz
                sum[1] +=  DD * yy;
                sum[2] +=  DD * zz;
                
               /*
                a[0][0] = R2 + DEL * DEL + xsq;
                a[1][1] = R2 + DEL * DEL + ysq;
                a[2][2] = R2 + DEL * DEL + zsq;
                a[0][1] = xx * yy;
                a[0][2] = xx * zz;
                a[1][2] = yy * zz;
                a[1][0] = a[0][1];
                ...
                sum[0] = sum[0] + (a[0][0] * ff[0] + a[0][1] * ff[1] + a[0][2]* ff[2]) * Rinv3;
                sum[1] = sum[1] + (a[1][0] * ff[0] + a[1][1] * ff[1] + a[1][2]* ff[2]) * Rinv3;
                sum[2] = sum[2] + (a[2][0] * ff[0] + a[2][1] * ff[1] + a[2][2]* ff[2]) * Rinv3;
                */
